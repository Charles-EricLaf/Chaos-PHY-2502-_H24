...
t_span_perturbed = [start, end]


# Équations différentielles non perturbées
...
for timepoint in range(start, end):
    ...
    # print(f"Vecteur :", y_vector)


    dc_dt1, dh_dt1 = three_cells_conc_perturbation(p, h1, conc1, conc2, conc3, D12, D13, pert)[0], InsP3(p, h1, conc1)[0]
    dc_dt2, dh_dt2 = three_cells_conc_perturbation(p, h2, conc2, conc1, conc3, D12, D23, pert)[0], InsP3(p, h2, conc2)[0]
    dc_dt3, dh_dt3 = three_cells_conc_perturbation(p, h3, conc3, conc1, conc2, D13, D23, pert)[0], InsP3(p, h3, conc3)[0]
...
plt.xlabel('Temps [s]')
plt.ylabel(r'Concentration de calcium ionisé [\mu M]')
plt.plot(temps, conc1_val, label=r"$c_1$", color="black", lw=2)
plt.xlim(0, 100)
# plt.ylim(0, 2.0)
plt.tick_params(direction="in")
plt.plot(temps, conc2_val, label=r"$c_2$", color="r", lw=2)
plt.plot(temps, conc3_val, label=r"$c_3$", color="b", lw=2)
